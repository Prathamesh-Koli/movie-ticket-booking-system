...
public interface ShowDao extends JpaRepository<Show, Long> {
	...
	@Query(value = """
			SELECT
			  ...
			  (SELECT COALESCE(SUM(COALESCE(sstp.price, s2.seat_price)), 0)
			     FROM show_seats ss2
			     ...
			     LEFT JOIN show_seat_type_prices sstp ON ss2.show_id = sstp.show_id AND s2.seat_type = sstp.seat_type
			     WHERE ss2.show_id = sh.show_id
			       AND r2.reservation_status = 'CONFIRMED') AS revenue,
			  (SELECT COALESCE(SUM(COALESCE(sstp2.price, s3.seat_price)), 0)
			     FROM show_seats ss3
			     ...
			     LEFT JOIN show_seat_type_prices sstp2 ON ss3.show_id = sstp2.show_id AND s3.seat_type = sstp2.seat_type
			...
			@Query(value = """
					SELECT
					  (SELECT COUNT(*) FROM theaters th WHERE th.owner_id = :ownerId AND th.status = 'PENDING') AS pendingTheaters,
					  (SELECT COUNT(*) FROM theaters th WHERE th.owner_id = :ownerId AND th.status = 'APPROVED') AS approvedTheaters,
					  (SELECT COUNT(*) FROM theaters th WHERE th.owner_id = :ownerId) AS totalTheaters,
					  ...
					  (SELECT COALESCE(SUM(COALESCE(sstp.price, s.seat_price)), 0)
					     ...
					     LEFT JOIN show_seat_type_prices sstp ON ss2.show_id = sstp.show_id AND s.seat_type = sstp.seat_type
					  ...
					  (SELECT COALESCE(SUM(COALESCE(sstp2.price, s2.seat_price)), 0)
					     ...
					     LEFT JOIN show_seat_type_prices sstp2 ON ss4.show_id = sstp2.show_id AND s2.seat_type = sstp2.seat_type
